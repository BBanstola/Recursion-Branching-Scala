...
def shortestPath(m:Array[Array[Int]],x:Int,y:Int, ex:Int, ey:Int):Int={
  if (x==ex && y==ey) 0
    ...
    maze(x)(y) = -2
    val answer =  (shortestPath(m,x+1,y,ex,ey) min shortestPath(m, x, y+1, ex,ey) min shortestPath(m,x-1,y,ex,ey) min shortestPath(m, x, y-1, ex,ey))+1
    maze(x)(y) = 0
    answer
  }
}
